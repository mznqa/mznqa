...
private:
	// 技能卡id-效果函数指针映射表
	static std::map<int, std::function<bool(int*, int)>> effectMap;
public:
	EffectSet();
	~EffectSet();
	...
	// 基础效果 - 影响血量
	// 参数：期望血量变化量
	static bool effectBase0(int *arr, int size);
	...
	// 特殊效果区域 //////////////////////////////////////////////////////////////////////////
	// 特殊效果 - 攻击无效
	static bool effectSpecial0(int *arr, int size);
	...
	// 条件效果 - 当前的血量是否处于[A]~[B]
	// 参数：范围下限，范围上限
	static bool effectCondition0(int *arr, int size);
